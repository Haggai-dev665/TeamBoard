...
flowchart TD
  ...
  C2 -->|NotificationService::createWelcomeNotification| S1[NotificationService]
...
flowchart TD
  ...
  C -->|Notice::create(author_id=auth)| N[(notices table)]
  C -->|NotificationService::notifyNewNotice| S[NotificationService]
...
flowchart TD
  ...
  C --> T{feedbackable_type?}
  T -->|notice| N1[Notice::findOrFail]
  T -->|document| D1[Document::findOrFail]

  C -->|optional attachment upload| UP[(storage/app/public/feedback/...)]
  C -->|Feedback::updateOrCreate per user+target| F[(feedback table)]

  C --> K{type == concern?}
  K -->|No| DONE[Back with success]
  K -->|Yes| SA[Find super admin by config(app.super_admin_email)]
...
flowchart TD
  ...
  C --> O{notification.user_id == auth()->id()?}
  O -->|No| ERR[403 Forbidden]
  O -->|Yes| UPD[notification.markAsRead()]
  UPD --> DB[(notifications.read = true)]
  DB --> OUT[Back / JSON success]
...
flowchart TD
  ...
  C2 -->|authorize('create', Employee::class)| P1[EmployeePolicy::create]
  ...
  C3 -->|NotificationService::notifyEmployeeAdded| S[NotificationService]
  ...
  P3 -->|delete photo if exists| FS[(storage/app/public/...)]
...
flowchart TD
  ...
  C2 -->|auth()->user()->update| U[(users table)]
  ...
  C3 -->|Hash::make + update| U2[(users table)]
